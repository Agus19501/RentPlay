...
function handleRentClick() {
    if (!isLoggedIn()) {
        showNotification('Inicia sesion para alquilar un juego');
        setTimeout(() => {
    ...
    // El flujo real de alquiler se confirma en el modal de pago.
    if (!document.getElementById('modal-rental')) {
        showNotification('Selecciona un metodo de pago para confirmar el alquiler');
    }
}
...
function isLoggedIn() {
    return Boolean(localStorage.getItem('rentplayToken'));
}
...
async function handleConfirmRental() {
    ...
    if (!selectedPaymentInput) {
        showNotification('Selecciona un metodo de pago');
        return;
    }
    ...
    if (!isLoggedIn()) {
        closeAllModals();
        showNotification('Inicia sesion para completar el alquiler');
        setTimeout(() => {
            window.location.href = 'login.html';
        }, 900);
        return;
    ...
    if (!api) {
        showNotification('No se encontro conexion con la API');
        return;
    }

    try {
        const priceValue = document.querySelector('.detail-value.price')?.textContent || '19.99 €';
        ...
        setTimeout(() => {
            window.location.href = 'mi-alquiler.html';
        }, 900);
    } catch (error) {
        showNotification(error.message || 'No se pudo registrar el alquiler en el servidor');
    }
}
