...
async function loadLatestRentalFromApi() {
    ...
    // Si no hay token de Agustin, crearemos uno fake temporal para que visualmente puedas probar su BD sin tener el login configurado en tus pantallas.
    let token = localStorage.getItem('rentplayToken');
    
    if (api && !token) {
        console.log("Simulando inicio de sesión transparente para enlazar BD...");
        try {
            await api.register({
    ...
    try {
        ...
        if (durationElement && Number.isFinite(latest.durationDays)) {
            ...
            // Esto dejaba tu span desmaquetado, mejor inyectamos si existe
        }
    } catch (error) {
        console.error('No se pudieron cargar alquileres reales de BD:', error.message);
    }
}
...
function handleViewDetails(e) {
    ...
    const gameTitle = gameCard ? gameCard.querySelector('.game-title').textContent : 'Detalles';
